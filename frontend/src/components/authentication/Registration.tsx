...
export const Registration: React.FC = observer((): React.ReactElement => {
  ...
  return (
    <Dialog maxWidth={"xs"} open={authenticationStore.registrationModal} onClose={handleClose}>
      <DialogTitle>Регистрация</DialogTitle>
      <DialogContent>
        <DialogContentText sx={error ? {color: "red"} : undefined}>
          {error || "Для полноценного доступа к сайту необходимо зарегистрироваться."}
        </DialogContentText>
        <Box
          component="form"
          noValidate
        ...
        >
          ...
          />
          <Button type={"submit"} sx={{mt: 3, mb: .5}} size={"large"} variant="outlined" fullWidth>Зарегистрироваться</Button>
        </Box>
      </DialogContent>
    </Dialog>
  )
